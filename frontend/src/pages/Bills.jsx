...
export default function Bills({ onTabChange }) {
  ...
  const filtered = bills.filter(
    (b) =>
      ...
      (b.ipd_no || '').toLowerCase().includes(search.toLowerCase())
  )

  const handleBillCreated = (newBill) => {
    setBills((prev) => [newBill, ...prev])
    showToast(`✓ Bill saved for ${newBill.patient_name} · Net ₹${parseFloat(newBill.net_bill).toLocaleString('en-IN')}`)
  }
  ...
  const handlePrint = (bill) => {
    ...
    const html = `<!DOCTYPE html><html><head><title>Bill – ${bill.patient_name}</title><style>
      ...
      .tot{margin-left:auto;width:260px;font-size:13px;border-collapse:collapse}
      ...
      <p style="font-size:16px;font-weight:700;margin-top:8px">PATIENT BILL</p></div>
      <div class="grid">
      ...
      <table><thead><tr><th>Charge</th><th style="text-align:right">Rate/Day</th>
        <th style="text-align:center">Days</th><th style="text-align:right">Amount</th></tr></thead>
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="max-w-2xl mx-auto w-full px-4 pt-4">
        <input
          ...
          placeholder="🔍  Search patient name or IPD no…"
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="font-medium">
              {search ? 'No matching bills' : 'No bills yet'}
            </p>
            <p className="text-sm mt-1">
              {search ? 'Try a different search term' : 'Tap + to create the first bill'}
            </p>
          </div>
        ) : (
          filtered.map((bill) => (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <p className="text-sm text-gray-500 mt-2 text-center">
              Delete the bill for{' '}
              <strong className="text-gray-700">{confirmDelete.patient_name}</strong>?
